...
from src.phase8_compression.compression_engine import CompressionEngine, CompressionConfig
...
def test_final_gate_failure_is_not_reported_as_success_and_ships_original():
    ...
    assert result.final_size_mb == result.original_size_mb, "reported a compressed size for the original model"
    assert abs(result.total_compression - 1.0) < 1e-6, "reported compression ratio != 1x while shipping original"
    assert getattr(result, "artifact_path", None) is None, "wrote a compressed artifact for unshipped weights"
