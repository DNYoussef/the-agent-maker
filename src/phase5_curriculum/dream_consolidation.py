    ...
    def consolidate(
        ...
        for epoch in range(self.num_epochs):
            ...
            for i in range(0, len(dreams), 8):  # Batch size 8
                batch = dreams[i:i+8]
                loss = self._consolidation_step(
                    model, optimizer, batch, tokenizer, device
                )
                total_loss += loss
                num_batches += 1
...
class DreamMetrics:
    ...
    def get_summary(self) -> Dict:
        ...
        return {
            ...
            'avg_consolidation_loss': sum(self.consolidation_loss) / max(1, len(self.consolidation_loss)),
