...
class Phase4Config:
    ...
    preserve_layers: List[str] = field(
    ...
    compression_targets: Dict[str, float] = field(
        default_factory=lambda: {
            "tiny": 6.0,  # <50M params
    ...
    sparsity_thresholds: Dict[str, float] = field(
        default_factory=lambda: {
            "tiny": 0.05,
            "small": 0.10,
            "medium": 0.15,
    ...
    wandb_tags: List[str] = field(default_factory=lambda: ["phase4", "bitnet", "compression"])
    ...
    def __post_init__(self):
        ...
        if self.quantization_bits != 1.58:
            raise ValueError("Phase 4 uses BitNet 1.58-bit quantization only")

        if self.calibration_samples < 100:
            raise ValueError("Calibration requires at least 100 samples")

        if not (0.0 <= self.max_accuracy_drop <= 0.5):
            raise ValueError("max_accuracy_drop must be between 0.0 and 0.5")
    ...
    def adapt_to_model_size(self, num_params: int):
        ...
        self.target_compression_ratio = self.compression_targets[size_category]
        self.sparsity_threshold = self.sparsity_thresholds[size_category]
