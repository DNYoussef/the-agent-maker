...
class Phase1Controller(PhaseController):
    ...
    def execute(self, input_models: Optional[List[Any]] = None) -> PhaseResult:
        ...
        tokenizer, processed_datasets = self._prepare_data()
        trained_models, all_metrics = self._train_models(processed_datasets, tokenizer)
    ...
    def _ensure_src_path() -> None:
        """Add src to sys.path (local import keeps path context for the phase)."""
        import sys
    ...
    def _prepare_data(self):
        """Set up the tokenizer and download+process the foundation datasets."""
        ...
        return tokenizer, processed_datasets
    ...
    def _train_models(self, processed_datasets, tokenizer):
        ...
        for spec in self.SPECIALIZATIONS:
            ...
            # Defaulting to 1 epoch / small batch for prototype speed unless specified.
            ...
            all_metrics[spec] = {
                "final_loss": (
                    trainer.best_val_loss if trainer.best_val_loss != float("inf") else 0.0
                ),
