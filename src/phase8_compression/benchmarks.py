...
class MMLUBenchmark:
    ...
    def format_prompt(self, question: Dict, few_shot_examples: List[Dict] = None) -> str:
    ...
    def evaluate(
        self, model: nn.Module, tokenizer: Any, questions: List[Dict] = None
    ) -> BenchmarkResult:
        """
        ...
        if questions is None:
            questions = self.get_questions(
                self.config.mmlu_subjects, self.config.mmlu_samples_per_subject
            )
...
class GSM8KBenchmark:
    ...
    def evaluate(
        ...
        few_shot = (
            self.SAMPLE_QUESTIONS[: self.config.few_shot_examples]
        ...
        with torch.no_grad():
            for q in questions:
                ...
                is_correct = pred_answer is not None and abs(pred_answer - true_answer) < 0.01
...
class BenchmarkSuite:
    ...
    def evaluate(
        self, model: nn.Module, tokenizer: Any, benchmarks: List[str] = None
    ...
    def compute_overall_score(
        self, results: Dict[str, BenchmarkResult], weights: Dict[str, float] = None
    ) -> float:
        """
        ...
        weighted_sum = sum(results[name].accuracy * weights.get(name, 1.0) for name in results)
    ...
    def compare_models(
        ...
        return CompressionBenchmarkResult(
            original_scores={name: r.accuracy for name, r in original_results.items()},
            compressed_scores={name: r.accuracy for name, r in compressed_results.items()},
