...
def render_reasoning_trace_tab() -> None:
    ...
    with col1:
        ...
        for i, (thought, rating) in enumerate(
            zip(
                trace["thoughts"] if isinstance(trace["thoughts"], list) else [],
                trace["quality_ratings"] if isinstance(trace["quality_ratings"], list) else [],
            )
        ):
    ...
    with col2:
        ...
        avg_quality = (
            np.mean(trace["quality_ratings"])
            if isinstance(trace["quality_ratings"], (list, np.ndarray))
            and len(trace["quality_ratings"]) > 0
            else 0.0
        )
        ...
        fig = go.Figure(
            go.Bar(
                x=[
                    ...
                    for i in range(
                        len(
                            trace.get("quality_ratings", [])
                            if isinstance(trace.get("quality_ratings"), list)
                            else []
                        )
                    )
                ],
