...
def create_curriculum_stages_visual(current_stage: int = 3):
    ...
    stages = [
        {"name": "Foundation", "level": 1, "questions": 2000, "difficulty": 2.5, "status": "complete"},
        {"name": "Basics", "level": 2, "questions": 2000, "difficulty": 3.5, "status": "complete"},
        {"name": "Intermediate", "level": 3, "questions": 2000, "difficulty": 5.0, "status": "in_progress"},
        ...
        {"name": "Frontier", "level": 7, "questions": 2000, "difficulty": 10.0, "status": "pending"},
    ]
    ...
    for i, stage in enumerate(stages):
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(
                size=marker_size,
                color=color,
                line=dict(color='#FFFFFF', width=3) if stage['status'] == 'in_progress' else dict(color='#2E3F4F', width=2),
                symbol='circle'
            ),
        ...
        completion = "100%" if stage['status'] == 'complete' else "75%" if stage['status'] == 'in_progress' else "0%"
        fig.add_annotation(
            x=i,
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            ticktext=[s['name'] for s in stages],
...
def create_adaptive_curriculum_chart(level_data: pd.DataFrame):
    ...
    colors = ['#00F5D4' if status == 'complete' else '#FF006E' if status == 'in_progress'
              else '#4A5568' for status in level_data['Status']]
...
def create_self_modeling_chart():
    ...
    fig.add_trace(go.Scatter(
        ...
        marker=dict(size=8, color='#8338EC', symbol='diamond', line=dict(color='#FFFFFF', width=1)),
...
def render_phase5_dashboard():
    ...
    st.markdown("**7-stage adaptive curriculum** → Edge-of-Chaos → Frontier models → Tool use → Ethics baking")
...
def render_config_panel():
    ...
    current_stage = st.slider(
        ...
        max_value=7,
        value=3,
        help="7-stage adaptive curriculum"
    )
    ...
    enable_dream = st.checkbox("Enable Dream Consolidation", value=True,
                              help="3 epochs per level with T=1.2 replay")
...
def render_curriculum_stages_tab():
    ...
    stage_details = pd.DataFrame({
        'Stage': ['1. Foundation', '2. Basics', '3. Intermediate', '4. Advanced',
                  '5. Expert', '6. Master', '7. Frontier'],
        ...
        'Cost': ['$60', '$70', '$80', '$95', '$110', '$130', '$155'],
        'Status': ['✓ Complete', '✓ Complete', '⏳ 75%', 'Pending', 'Pending', 'Pending', 'Pending']
    })
...
def render_edge_of_chaos_tab():
    ...
    adjustment_data = pd.DataFrame({
        ...
        'Adjustment': ['↑ +0.5', '↑ +0.5', '↑ +0.5', '↑ +0.5', '↑ +0.5', '↑ +0.2', '↑ +0.3', '→ 0', '↓ -0.2', '↑ +0.2']
    })

    fig_adjustment = go.Figure()

    fig_adjustment.add_trace(go.Scatter(
