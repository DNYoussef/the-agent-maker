...
# Import wandb logging utilities
try:
    from agent_forge.utils.wandb_logger import PhaseLogger
except ImportError:
    PhaseLogger = None
...
class VPTQCompressor:
    ...
    def __init__(self, bits: int = 2, vector_dim: int = 4, iterations: int = 10):
        ...
        logger.info(
            f"VPTQ initialized: {bits} bits, codebook={self.codebook_size}, vector_dim={vector_dim}"
        )
...
class HyperCompressionEncoder:
    ...
    def __init__(
        ...
        logger.info(
            f"HyperCompression initialized: {num_clusters} clusters, "
            f"trajectories: {self.trajectory_types}"
        )
    ...
    def compress(self, weights: torch.Tensor, trajectory_type: str = "auto") -> dict[str, Any]:
        ...
        if trajectory_type == "auto":
            for i, cluster in enumerate(clusters):
                ...
                logger.debug(
                    f"Cluster {i}: {best_params['trajectory_type']} "
                    f"(error: {best_params['err']:.6f})"
                )
        else:
...
class GrokfastCompressionOptimizer:
    ...
    def __init__(
        self, ema_alpha: float = 0.98, grokfast_lambda: float = 0.05, learning_rate: float = 0.001
    ):
...
class FinalCompressionConfig(BaseModel):
    ...
    hyper_trajectory_types: list[str] = Field(
        default=["sinusoidal", "spiral", "chaotic"], min_items=1
    )
    ...
    compression_order: list[str] = Field(
        default=["seedlm", "vptq", "hypercompression"], min_items=1
    )
    ...
    wandb_project: str = "agent-forge-pipeline"
    wandb_entity: Optional[str] = None
    wandb_tags: list[str] = Field(default=["final_compression", "phase8", "compression"])
...
class FinalCompressionPhase(PhaseController):
    ...
    def __init__(self, config: FinalCompressionConfig, pipeline_id: Optional[str] = None):
        self.config = config
        self.logger = logging.getLogger(__name__)
        self.pipeline_id = pipeline_id
        self.wandb_logger: Optional[PhaseLogger] = None

        # Initialize compressors based on configuration
        if config.enable_seedlm:
            self.seedlm = SEEDLMCompressor(
                bits_per_weight=config.seedlm_bits_per_weight,
                max_candidates=config.seedlm_max_candidates,
            )

        if config.enable_vptq:
            self.vptq = VPTQCompressor(
                bits=config.vptq_bits,
                vector_dim=config.vptq_vector_dim,
                iterations=config.vptq_iterations,
            )

        if config.enable_hypercompression:
            self.hypercompression = HyperCompressionEncoder(
                num_clusters=config.hyper_num_clusters,
                trajectory_types=config.hyper_trajectory_types,
            )
    ...
    async def run(self, model: nn.Module) -> PhaseResult:
        ...
        if self.config.wandb_enabled and PhaseLogger is not None:
            self.wandb_logger = PhaseLogger(
                phase_name="phase8-final-compression",
                config={
                    "enable_seedlm": self.config.enable_seedlm,
                    "enable_vptq": self.config.enable_vptq,
                    "enable_hypercompression": self.config.enable_hypercompression,
                    "seedlm_bits_per_weight": self.config.seedlm_bits_per_weight,
                    "vptq_bits": self.config.vptq_bits,
                    "vptq_vector_dim": self.config.vptq_vector_dim,
                    "hyper_num_clusters": self.config.hyper_num_clusters,
                    "enable_grokfast_optimization": self.config.enable_grokfast_optimization,
                    "grokfast_ema_alpha": self.config.grokfast_ema_alpha,
                    "grokfast_lambda": self.config.grokfast_lambda,
                    "compression_order": self.config.compression_order,
        ...
        try:
            ...
            if self.wandb_logger:
                self.wandb_logger.log_metrics({
                    "compression/original_size_mb": original_size / 1e6,
                    "compression/num_weight_tensors": len(original_weights),
                }, step=0)
            ...
            for idx, method in enumerate(self.config.compression_order, start=1):
                ...
                if self.wandb_logger:
                    stage_metrics = stage_results["metrics"]
                    self.wandb_logger.log_metrics({
                        f"{method}/compression_ratio": stage_metrics.get("compression_ratio", 0),
                        f"{method}/original_size_mb": stage_metrics.get("original_size_mb", 0),
                        f"{method}/compressed_size_mb": stage_metrics.get("compressed_size_mb", 0),
                    }, step=idx)
            ...
            if self.grokfast_optimizer:
                ...
                for method in self.config.compression_order:
                    if method in compressed_weights:
                        ...
                        if self.wandb_logger:
                            self.wandb_logger.log_metrics({
                                f"grokfast/{method}/optimized_score": opt_result.get("optimization_score", 0),
                            }, step=len(self.config.compression_order) + 1)

            # Stage 3: Validate compression quality
            validation_results = {}
            if self.config.validate_compression:
                self.logger.info("Validating compression quality")
                validation_results = await self._validate_compression(
                    original_weights, compressed_weights
                )

                # Log validation results
                if self.wandb_logger:
                    for method, validation in validation_results.items():
                        self.wandb_logger.log_metrics({
                            f"validation/{method}/avg_mse": validation.get("avg_mse", 0),
                            f"validation/{method}/avg_mae": validation.get("avg_mae", 0),
                            f"validation/{method}/avg_relative_error": validation.get("avg_relative_error", 0),
                            f"validation/{method}/within_tolerance": int(validation.get("within_tolerance", False)),
                        }, step=len(self.config.compression_order) + 2)
            ...
            if self.wandb_logger:
                self.wandb_logger.log_summary({
                    ...
                    "phase/original_size_mb": original_size / 1e6,
                    "phase/compressed_size_mb": compressed_size / 1e6,
                    "phase/total_compression_ratio": total_compression_ratio,
                    "phase/methods_used": len(compressed_weights),
                    "phase/validation_passed": all(
                        v.get("within_tolerance", False) for v in validation_results.values()
            ...
            if self.wandb_logger:
                ...
                self.wandb_logger.finish(exit_code=1)
    ...
    if result.success:
        ...
        if "validation_results" in metrics:
            ...
            for method, validation in metrics["validation_results"].items():
                ...
                print(
                    f"  {method.upper()}: {status} (error: {validation['avg_relative_error']:.4f})"
                )
    else:
