#!/usr/bin/env python3
"""
EvoMerge V7 - ACCURACY-FIRST Evolution

Changes from V6:
1. FITNESS FORMULA: Accuracy is now PRIMARY driver (70% weight)
   - V6: speed dominated (0.77 contribution vs 0.012 for accuracy)
   - V7: accuracy^0.5 amplifies small gains, speed minimized to 10%
2. CONVERGENCE: Increased patience from 5 to 10 generations
3. OUTPUT: Saves to checkpoints/phase2_v7/
...
class RealBenchmarker:
    ...
    def evaluate_full(self, model: nn.Module) -> Dict[str, Any]:
        ...
        # ============================================================
        # V7 ACCURACY-FIRST FITNESS FORMULA
        # ============================================================
        # V6 Problem: speed dominated (0.77 contribution vs 0.012 for accuracy)
        # V7 Fix: accuracy is PRIMARY driver
        #
        # Math check:
        #   4% accuracy: 0.70 * 0.2 + 0.10 * 0.9 = 0.14 + 0.09 = 0.23
        #   0% accuracy: 0.70 * 0.0 + 0.10 * 0.9 = 0.00 + 0.09 = 0.09
        # Now 4% accuracy model WINS over 0% fast model!
        # ============================================================

        # Amplify small accuracy gains with sqrt (0.04 -> 0.2)
        acc_score = accuracy ** 0.5 if accuracy > 0 else 0

        # Perplexity: lower is better, but cap influence
        ppl_score = max(0, 1.0 - (perplexity / 1000))

        # Speed: normalized 0-1, capped at 5000 tok/s, MINIMAL weight
        speed_score = min(1.0, speed / 5000)

        # Memory: smaller is better
        mem_score = min(1.0, 500 / max(1, memory))

        # ACCURACY-DOMINANT: 70% accuracy, 15% perplexity, 10% speed, 5% memory
        fitness = 0.70 * acc_score + 0.15 * ppl_score + 0.10 * speed_score + 0.05 * mem_score
...
def run_evomerge_v7(
    ...
    output_dir: str = "checkpoints/phase2_v7",
    generations: int = 50,
    gsm8k_samples: int = 50,
    ppl_samples: int = 100,
    ...
    convergence_patience: int = 10,  # V7: Increased from 5
    device: str = "cuda"
):
    """
    Run EvoMerge V7 with ACCURACY-FIRST fitness formula.
    """

    print("=" * 70)
    print("EVOMERGE V7 - ACCURACY-FIRST EVOLUTION")
    print("=" * 70)
    print(f"Generations: {generations}")
    print(f"GSM8K samples: {gsm8k_samples}")
    print(f"Perplexity samples: {ppl_samples}")
    print(f"Convergence patience: {convergence_patience} generations")
    print("")
    print("FITNESS FORMULA (V7):")
    print("  70% accuracy (sqrt amplified)")
    print("  15% perplexity")
    print("  10% speed")
    print("  5% memory")
    print("=" * 70)
    ...
    for gen in range(generations):
        ...
        if fitness_scores[0] > champion_fitness:
            ...
            champion_accuracy = accuracy_scores[0]
            torch.save({
        ...
        accuracy_history.append(max(accuracy_scores))
        diversity_history.append(diversity)

        improvement = (champion_fitness / fitness_history[0] - 1) * 100 if fitness_history[0] > 0 else 0

        print(f"\n  Gen {gen + 1} Summary:")
        print(f"    Best fitness: {fitness_scores[0]:.4f}")
        print(f"    Best accuracy: {max(accuracy_scores):.1f}%")
        print(f"    Champion: fit={champion_fitness:.4f}, acc={champion_accuracy:.1f}% ({improvement:+.1f}%)")
        ...
        # Early stopping (V7: increased patience to 10)
        if gen >= convergence_patience:
            recent = fitness_history[-convergence_patience:]
...
if __name__ == "__main__":
    run_evomerge_v7(
        ...
        output_dir="checkpoints/phase2_v7",
        generations=50,
        gsm8k_samples=50,
        ppl_samples=100,
        mutation_sigma=0.01,
